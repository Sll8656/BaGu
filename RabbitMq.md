...
publicsher `->exchange ->queue-> `consumer
...
3. 消费者没接受到消息：
   ...
   - 消费者确认机制，消费者消费完消息给mq发送确认。

   - 失败重试机制：消费出现异常会重试，次数到达上限后，消息被投递进异常交换机，由人工处理。

​	消费者丢数据一般是因为采用了自动确认消息模式，改为手动确认消息即可！

​	消费者在收到消息之后，处理消息之前，会自动回复RabbitMQ已收到消息。如果这时处理消息	失败，就会丢失该消息；
...
发送消息时让每个消息携带一个**全局的唯一ID**，然后利用一张日志表来记录已经处理成功的消息的 ID，如果新到的消息 ID 已经在日志表中，那么就不再处理这条消息。
