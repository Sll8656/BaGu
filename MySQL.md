...
**B+ 树的非叶子节点不存放实际的记录数据，仅存放索引，因此页面大小相同的情况下，相比即存索引又存记录的 B 树，B+树的非叶子节点可以存放更多的索引，因此 B+ 树可以比 B 树更「矮胖」，查询底层节点的磁盘 I/O次数会更少。**
...
​	当事务准备提交时，必须先确保所有相关的日志记录都已经持久化到磁盘。**先将日志持久化到磁盘，再提交事务**
...
* 隔离性：数据库系统提供的隔离机制，保证：\其他事务不会看到该事务的中间状态，事务在**不受外部并发操作影响**的独立环境下运行
