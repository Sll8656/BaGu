...
![image-20240406194725533](./assets/image-20240406194725533.png)
...
* **保证线程间的可见性**
  ...
  ![image-20240406195726265](./assets/image-20240406195726265.png)

  这段代码线程2能够打印stop为true，但是线程3中stop却一直是false导致空转。

  问题分析：主要是因为JVM中的JIT（即时编译器）给代码做了优化。

  原来的`while(!stop)`被优化成了`while(true)`。

  解决方案：

  * 在vm参数中输入`-Xint`来禁用即时编译器
  * 用**volatile**修饰stop（stop变量变成了易变的变量），jit就不会对volatile修饰的变量做优化
...
![image-20240406201845555](./assets/image-20240406201845555.png)

如果变量x加上volatile，是无法解决问题的。原因：

![image-20240407121635494](./assets/image-20240407121635494.png)
...
<img src="./assets/image-20240407122617977.png" style="zoom:80%;" />
...
* JDK1.8采用数组+链表/红黑树
  ...
  * synchronized只锁定当前链表或红黑二叉树的**首节点**，只要hash不冲突，就不会产生并发问题，效率得到提升
  ...
  ![image-20240407150501160](./assets/image-20240407150501160.png)
...
![image-20240407161247926](./assets/image-20240407161247926.png)
